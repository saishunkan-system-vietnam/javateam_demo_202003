...
public class StudentReponsitory {
	...
	/*
	 * public List<StudentDTO> search(Student student) { return
	 * mapper.search(student).stream().map(student1 -> new
	 * StudentDTO(student1)).collect(Collectors.toList()); }
	 */
	...
	public List<StudentDTO> findStudent(Student student) {
		return mapper.findStudent(student).stream().map(student2 -> new StudentDTO(student2))
	...
	public int findCountStudents(StudentDTO student) {
		return mapper.findCountStudents(student);
	}
}
